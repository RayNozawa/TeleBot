    ...
    if (data.dead.length < 1) {
        ...
        for (let i = 0; i < data.dead.length; i++) {
            ...
            if (data.player[b].effect.includes("guardian")) {
                e.push(data.player[b].name);
            } else {
                f.push(data.player[b].name);
            }
        }
...
async function pagi(conn, x, data) {
    ...
    return await conn.sendPhoto(x.room, thumb1, { caption: pagii(x) })
}
...
async function voting(conn, x, data) {
    ...
    shortPlayer(x.room, data);
    for (let i = 0; i < x.player.length; i++) {
        textnya += `(${x.player[i].number}) ${x.player[i].name} ${x.player[i].isdead === true ? "☠️" : ""}\n`;
        ment.push(x.player[i].id);
    }
    ...
    return await conn.sendPhoto(x.room, thumb2, { caption: textnya })
}
...
async function malam(conn, x, data) {
    ...
    if (hasil_vote === 0) {
        ...
        return conn.sendPhoto(x.room, thumb3, { caption: textnya })
            .then(() => {
                changeDay(x.room, data);
        ...
        if (hasil_vote.role === "werewolf") {
            textnya = `*⌂ W E R E W O L F - G A M E*\n\nWarga desa telah memilih dan sepakat ${hasil_vote.name} dieksekusi mati.\n\n${hasil_vote.name} adalah ${hasil_vote.role} ${emoji_role(hasil_vote.role)}`;
            voteKill(x.room, data);
            let ment = [];
            ment.push(hasil_vote.id);
            return await conn.sendPhoto(x.room, thumb4, { caption: textnya })
                .then(() => {
                    changeDay(x.room, data);
                    voteDone(x.room, data);
                    resetVote(x.room, data);
            ...
            textnya = `*⌂ W E R E W O L F - G A M E*\n\nWarga desa telah memilih dan sepakat ${hasil_vote.name} dieksekusi mati.\n\n${hasil_vote.name} adalah ${hasil_vote.role} ${emoji_role(
        hasil_vote.role
...
async function skill(conn, x, data) {
    ...
    if (getWinner(x.room, data).status != null || x.win != null) {
        ...
        shortPlayer(x.room, data);
        for (let i = 0; i < x.player.length; i++) {
            tok1 += `(${x.player[i].number}) ${x.player[i].name}${x.player[i].isdead === true ? " ☠️" : ""}\n`;
            membernya.push(x.player[i].id);
        }
        for (let i = 0; i < x.player.length; i++) {
            tok2 += `(${x.player[i].number}) ${x.player[i].name} ${
        ...
        for (let i = 0; i < x.player.length; i++) {
            if (x.player[i].role === "werewolf") {
                if (x.player[i].isdead != true) {
                    ...
                    await conn.reply(x.player[i].id, textnya)
                }
    ...
    if (getWinner(x.room, data).status === false || x.iswin === false) {
        ...
        for (let i = 0; i < x.player.length; i++) {
            if (x.player[i].role === "sorcerer" || x.player[i].role === "werewolf") {
                textnya += `${x.player[i].number}) ${x.player[i].name}\n     *Role* : ${x.player[i].role}\n\n`;
                ment.push(x.player[i].id);
            }
        }
        return await conn.sendPhoto(sesinya, thumb5, { caption: textnya })
            .then(() => {
                delete data[x.room];
            });
        ...
        return await conn.sendPhoto(sesinya, thumb6, { caption: textnya })
    }
}
