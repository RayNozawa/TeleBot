...
export const run = {
  async: async (m, { conn, Api, body, Func, users, env, isROwner, isPrefix }) => {
    ...
    if (m.text == (isPrefix + "hlogo")) return conn.reply(m.chat, `<pre><code class="language-Clue">${json.jawaban.replace(/[AIUEOaiueo]/ig, '_')}</code></pre>`, m.msg, "HTML")

    if (!m.quoted || !m.text || !/Ketik.*hlogo/i.test(m.quoted.text) || /.*hgamb/i.test(m.text)) {
    ...
    if (m.quoted.id == conn.tebaklogo[id][0].message_id) {
        ...
        if (isSurrender) {
            ...
            clearTimeout(conn.tebaklogo[id][2])
        ...
        if (m.text.toLowerCase() == json.jawaban.toLowerCase().trim()) {
            ...
            await conn.reply(m.chat, caption, m.msg, "Markdown", [[{ text: 'Mainkan Lagi🎮', callback_data: '.tebaklogo' }]])
            clearTimeout(conn.tebaklogo[id][2])
            delete conn.tebaklogo[id]
